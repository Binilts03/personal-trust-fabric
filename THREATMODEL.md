...
- Concurrent redemption races. Capability: two writers redeem the same single-use approval/cap simultaneously. Mitigation: documented single-writer rule (one server per store or external locking). Gap: last-write-wins lost update is the default — double-spend/double-redeem is a deployment bug, not a prevented attack.
