...
// only and refuses to start without an API key. Single endpoint:
// POST /access/v1/evaluation (boxcarred evaluations + search APIs are out of
// scope). Deny is 200 + {decision:false}; 401 means the PEP failed PDP auth,
// never a denied subject. Malformed bodies are 400 transport errors —
// fail-closed, never a decision.
...
const server = createServer((req, res) => {
  ...
  req.on("close", () => {
    ...
    try {
      ...
      json(res, 400, { error: "malformed body" });
      return;
    }
    ...
    try {
      ...
      decision = evaluateAuthZen(auth, body, {
        nowSec: Math.floor(Date.now() / 1000),
      });
    } catch {
      json(res, 400, { error: "malformed evaluation request" });
      return;
    }
    json(res, 200, { decision: decision.decision, context: decision.context });
  });
});
