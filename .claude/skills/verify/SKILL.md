...
2. **Drive the change:** spawn a fresh verifier sub-agent (or a clean shell) that exercises the changed seam from `src/index.ts` — never internals (bin entries `src/cli.ts`, `src/mcp-server.ts` exempt per harness rule) — with a script under `/tmp` or `evidence/`. For capability work: issue → attenuate → dry-run → redeem-with-proof → assert receipt fields, plus one abuse case (replay / wrong recipient / mutated digest).
